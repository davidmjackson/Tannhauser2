...
- **Git auth note:** pushes authenticate as the **davidmjackson** account via Git Credential Manager (stored in Windows Credential Manager). The old `x-access-token` token credential was deleted on 2026-06-28 because having both it and the account credential made GCM prompt to choose an identity on every push. The `gh` CLI has its own separate login (used for PRs). If a future push asks to re-authenticate, just re-auth the account.
